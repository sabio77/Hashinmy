(function () {
  ...
  async function applyPublishedVersionChange(versionPayload = {}, options = {}) {
  ...
  function normalizeRealtimeUpdateDetail(detail = {}) {
    if (!detail || typeof detail !== 'object') return { type: '', data: {} };
    return {
      type: String(detail.type || detail.eventType || detail.tipo || '').trim(),
      data: detail.data && typeof detail.data === 'object' ? detail.data : detail
    };
  }

  function resolveRealtimeVersionPayload(detail = {}) {
    const normalized = normalizeRealtimeUpdateDetail(detail);
    ...
    for (const candidate of candidates) {
      const version = getPublishedVersionFromPayload(candidate)
        || String(candidate.logoVersion || candidate.assetVersion || candidate.cacheVersion || '').trim();
      if (version) return { ...candidate, version };
    }

    return normalized.type ? { version: '' } : null;
  }

  async function applyRealtimeUpdatePayload(detail = {}) {
    const normalized = normalizeRealtimeUpdateDetail(detail);
    const versionPayload = resolveRealtimeVersionPayload(normalized) || { version: '' };

    if (state.versionCheckInFlight) return state.versionCheckInFlight;

    state.lastVersionCheckAt = Date.now();
    state.lastVersionCheckReason = normalized.type || 'memoriaBACKEND-event';
    state.versionCheckInFlight = (async () => {
      try {
        if (!state.registration) {
          const registrationResult = await registerServiceWorker();
          if (!registrationResult.ok && !state.registration) return registrationResult;
        }

        if (versionPayload.version && state.lastKnownVersion && versionPayload.version === state.lastKnownVersion && !/logo|asset|brand/i.test(normalized.type)) {
          return { ok: true, changed: false, version: versionPayload.version, message: 'La versión publicada ya está aplicada.' };
        }

        return await applyPublishedVersionChange(versionPayload, { reason: normalized.type || 'memoriaBACKEND-event', force: true });
      } catch (error) {
        return { ok: false, message: error?.message || 'No se pudo aplicar la actualización recibida por evento.' };
      } finally {
        state.versionCheckInFlight = null;
      }
    })();
  ...
  function drainQueuedRealtimeUpdates() {
    const queue = Array.isArray(window.__ChatERPWARealtimeUpdates) ? window.__ChatERPWARealtimeUpdates.splice(0) : [];
    queue.forEach((detail) => {
      applyRealtimeUpdatePayload(detail).catch(() => null);
    });
  }
  ...
  function startVersionWatch(registration) {
    ...
    window.addEventListener('chater:pwa-realtime-update', (event) => {
      applyRealtimeUpdatePayload(event.detail || {}).catch(() => null);
    });

    navigator.serviceWorker?.addEventListener('message', (event) => {
      if (event.data?.type === 'STATIC_APP_UPDATE_AVAILABLE') {
        applyRealtimeUpdatePayload({ type: 'service-worker.update', data: event.data }).catch(() => null);
      }
    });

    registration.update().catch?.(() => null);
    drainQueuedRealtimeUpdates();
  }
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      return { ok: true, message: 'Modo instalable activo y conectado a actualizaciones por eventos de memoriaBACKEND.' };
    } catch (error) {
  ...
  window.ChatERPWA = {
    ...
    async checkForUpdates() {
      ...
      if (state.registration.waiting) {
        state.waitingWorker = state.registration.waiting;
        if (applyWaitingUpdate()) return { ok: true, changed: true, message: 'Actualización activada automáticamente.' };
