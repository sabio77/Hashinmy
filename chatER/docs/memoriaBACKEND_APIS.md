...
## 130. Revisión de punto débil: actualización total por eventos sin polling hacia memoriaBACKEND

En esta iteración se corrigió el punto débil principal de ChatER instalado: la app podía detectar cambios de shell mediante verificación periódica de `app-version.json` y todavía mostraba un botón de actualización, pero la solicitud exige que ChatER se mantenga actualizado automáticamente cuando memoriaBACKEND emita eventos, sin reinstalar y sin presionar botones.

Cambios aplicados:

- `js/pwa.js` deja de iniciar vigilancia periódica con `setInterval` para buscar nuevas versiones. La actualización del shell instalado pasa a ser event-driven.
- `js/app.js` escucha eventos STREMEx/SSE/WebSocket de memoriaBACKEND para `app.version.changed`, `static.version.changed`, `static-site.updated`, `chater.updated`, `app.logo.updated` y `brand.logo.updated`.
- Cuando llega un evento de versión o logo, `js/app.js` notifica a `window.ChatERPWA`, que refresca `index.html`, `manifest.json`, `app-version.json`, CSS, JS, iconos y logo desde el service worker, aplica `SKIP_WAITING` si hay worker en espera y recarga automáticamente.
- `service-worker.js` agrega `assets/chater-logo.png` a los assets opcionales refrescables, con fallback geométrico si el PNG aún no existe en `assets`, manteniendo la regla de imágenes del proyecto.
- `js/app.js` procesa eventos de conversaciones/chats creados, actualizados, archivados, restaurados y eliminados; perfiles/avatares actualizados; estados creados, actualizados y eliminados; llamadas entrantes y llamadas actualizadas.
- Las fotos de perfil remotas ahora pueden venir como URL segura `http(s)` desde memoriaBACKEND o como PNG dentro de `assets`, sin romper los placeholders locales existentes.

Contrato de eventos esperado desde memoriaBACKEND por STREMEx:

```txt
conversation.created | conversation.updated | conversation.archived | conversation.restored | conversation.deleted | conversation.pinned | conversation.unpinned
chat.created | chat.updated | chat.archived | chat.restored | chat.deleted | chat.pinned | chat.unpinned
message.created | message.updated | message.deleted | message.delivered | message.read
profile.updated | user.profile.updated | avatar.updated | profile.avatar.updated
state.created | state.updated | state.deleted | status.created | status.updated | status.deleted
call.incoming | call.updated
presence.changed | typing.started | typing.stopped
app.version.changed | static.version.changed | static-site.updated | chater.updated
app.logo.updated | brand.logo.updated
```
...
- No se modifican autenticación Google/Gmail, compresión WebP, QR, permisos, cola durable, envío de mensajes, adjuntos, R2, llamadas manuales, estados locales ni configuración base de memoriaBACKEND fuera de la sincronización realtime y autoactualización instalada.
