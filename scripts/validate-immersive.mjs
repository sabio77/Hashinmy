...
if (!assetsOnly) {
  ...
  assert(jsText.includes("const MEMORIA_BACKEND_SITE_ID = 'a1';") && jsText.includes('/sdk/memoria.js?s=') && jsText.includes('function resolveLanguageFromMemoriaBackendVisitOpening') && jsText.includes('api.registrarAperturaStatica') && jsText.includes('api.detectarPaisEIdioma') && jsText.includes('api.registerStaticOpen') && jsText.includes('requestVisitOpeningApiDirectly') && jsText.includes('/visitas/apertura') && !jsText.includes(['geo', 'Visit', 'Api'].join('')) && !jsText.includes(`/${['geo', 'Visit', 'Api'].join('')}`), 'La apertura debe consultar la API oficial POST /api/v1/visitas/apertura de memoriaBACKEND con el site autorizado a1 antes de caer al idioma del navegador; no debe usar el endpoint legado de geovisita ni endpoints alternos.');
