...
if (!assetsOnly) {
  ...
  assert(jsText.includes("const MEMORIA_BACKEND_SITE_ID = 'a1';") && jsText.includes('/sdk/memoria.js?s=') && jsText.includes('function resolveLanguageFromMemoriaBackendVisitApi') && jsText.includes('window.memoriaBACKEND.geoVisitApi') && jsText.includes('requestGeoVisitApiDirectly') && jsText.includes('/geoVisitApi'), 'La apertura debe consultar geoVisitApi de memoriaBACKEND con el site autorizado a1 antes de caer al idioma del navegador.');
  assert(jsText.includes('const inlineLanguage = getResolvedTextBundleLanguage(inlineBundle') && jsText.includes('if (inlineLanguage === preferred) return inlineBundle;') && !jsText.includes("const spanishCachedBundle = readCachedTextBundle('es');"), 'El arranque rápido no debe pintar el bundle español embebido cuando geoVisitApi o el navegador resolvieron otro idioma; debe cargar el bundle solicitado antes del primer render.');
