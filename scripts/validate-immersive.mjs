...
const REQUIRED_CHATER_SOCIAL_PREVIEW_TITLE = 'ChatER | Chat web conectado a memoriaBACKEND';
const REQUIRED_CHATER_SOCIAL_PREVIEW_DESCRIPTION = 'Chat web tipo WhatsApp para sitio estático conectado a APIs memoriaBACKEND.';
const REQUIRED_CHATER_SOCIAL_PREVIEW_TEXT = `${REQUIRED_CHATER_SOCIAL_PREVIEW_TITLE}. ${REQUIRED_CHATER_SOCIAL_PREVIEW_DESCRIPTION}`;
const REQUIRED_CHATER_PUBLIC_URL = `${PUBLIC_SITE_URL}chatER/`;
...
const chaterIndexHtml = await readRelative('chatER/index.html');
...
if (!assetsOnly) {
  ...
  assert(
    ...
    'index.html debe publicar miniVISTA.png como única imagen social, incluyendo og:image, image_src, thumbnail, itemprop y twitter:image sin fallback a otros assets.'
  );
  assert(!indexHtml.includes('data-hashinmy-preview-fallback'), 'index.html no debe declarar imagen social fallback: miniVISTA.png debe ser la única miniatura para WhatsApp y otros previsualizadores.');
  assertOnlyMiniVistaSocialPreviewImages(indexHtml, 'index.html');
  assert(
    chaterIndexHtml.includes(`property="og:title" content="${REQUIRED_CHATER_SOCIAL_PREVIEW_TITLE}"`)
      && chaterIndexHtml.includes(`property="og:description" content="${REQUIRED_CHATER_SOCIAL_PREVIEW_DESCRIPTION}"`)
      && chaterIndexHtml.includes(`property="og:url" content="${REQUIRED_CHATER_PUBLIC_URL}"`)
      && chaterIndexHtml.includes(`property="og:image" content="${REQUIRED_SOCIAL_PREVIEW_IMAGE}"`)
      && chaterIndexHtml.includes(`property="og:image:secure_url" content="${REQUIRED_SOCIAL_PREVIEW_IMAGE}"`)
      && chaterIndexHtml.includes('property="og:image:type" content="image/png"')
      && chaterIndexHtml.includes('property="og:image:width" content="1200"')
      && chaterIndexHtml.includes('property="og:image:height" content="630"')
      && chaterIndexHtml.includes(`property="og:image:alt" content="${REQUIRED_CHATER_SOCIAL_PREVIEW_TEXT}"`)
      && chaterIndexHtml.includes(`rel="image_src" href="${REQUIRED_SOCIAL_PREVIEW_IMAGE}"`)
      && chaterIndexHtml.includes(`name="thumbnail" content="${REQUIRED_SOCIAL_PREVIEW_IMAGE}"`)
      && chaterIndexHtml.includes(`itemprop="image" content="${REQUIRED_SOCIAL_PREVIEW_IMAGE}"`)
      && chaterIndexHtml.includes(`name="twitter:image" content="${REQUIRED_SOCIAL_PREVIEW_IMAGE}"`)
      && chaterIndexHtml.includes(`name="twitter:image:src" content="${REQUIRED_SOCIAL_PREVIEW_IMAGE}"`)
      && chaterIndexHtml.includes(`name="twitter:image:alt" content="${REQUIRED_CHATER_SOCIAL_PREVIEW_TEXT}"`)
      && chaterIndexHtml.includes(`rel="canonical" href="${REQUIRED_CHATER_PUBLIC_URL}"`),
    'chatER/index.html debe publicar miniVISTA.png como única miniatura para WhatsApp y otros previsualizadores; los iconos PWA no deben ser la imagen social.'
  );
  assertOnlyMiniVistaSocialPreviewImages(chaterIndexHtml, 'chatER/index.html');
  {
  ...
  assert(indexHtml.includes(`type="text/plain" title="Hashinmy llms.txt" href="${PUBLIC_SITE_URL}llms.txt"`), 'index.html debe exponer llms.txt desde el head como señal directa para rastreadores de IA.');
  ...
  const initSource = jsText.slice(jsText.indexOf('async function init()'), jsText.indexOf('function runInit()'));
  assert(jsText.includes('function bindImmediateSeoEntryAccess()') && jsText.includes("elements.seoHubButton.addEventListener('pointerdown', handleSeoHubEntryPointerDown, { passive: true })") && jsText.includes("elements.seoHubButton.addEventListener('keydown', handleSeoHubEntryKeyDown)") && jsText.includes("elements.seoHubButton.addEventListener('click', handleSeoHubEntryClick)") && jsText.includes('function handleSeoHubEntryPointerDown(event)') && jsText.includes("if (event.key === 'Enter' || event.key === ' ') event.stopPropagation();") && initSource.includes('bindImmediateSeoEntryAccess();') && initSource.includes('const memoriaBackendRuntimeConfigReady = loadMemoriaBackendRuntimeConfig();') && initSource.includes('await memoriaBackendRuntimeConfigReady;') && initSource.indexOf('bindImmediateSeoEntryAccess();') < initSource.indexOf('loadMemoriaBackendRuntimeConfig();') && initSource.indexOf('bindImmediateSeoEntryAccess();') < initSource.indexOf('await memoriaBackendRuntimeConfigReady;') && initSource.indexOf('bindImmediateSeoEntryAccess();') < initSource.indexOf('await readLanguage();'), 'Productos debe enlazar su acceso dedicado antes de cualquier espera remota de CONFIGmemoriaBACKEND o idioma/visita y aislar pointerdown/teclado del primer intento global de fullscreen, para que el primer click de escena 1 no dependa del bootstrap general.');
  assert(jsText.includes("const fallbackHref = actionElement?.href || '';") && jsText.includes("Productos no pudo abrir el hub inmediato; se usa la ruta navegable de respaldo.") && jsText.includes('if (fallbackHref) window.location.href = fallbackHref;'), 'Productos debe conservar una navegación de respaldo si la hidratación local del hub falla, evitando clicks silenciosos.');
  assert(!jsText.includes('function isSeoEntryEnhancedNavigationReady()') && jsText.includes("function handleSeoHubEntryClick(event) {\n    event.preventDefault();\n    event.stopPropagation();\n    openSeoHubFromEntry(event.currentTarget);\n  }"), 'Productos debe interceptar su primer click desde que se enlaza el acceso dedicado, sin esperar appReady, y abrir el shell del hub inmediatamente; el href queda solo como respaldo si la apertura del hub falla.');
  assert(jsText.includes('seoOpenRequestToken: 0') && jsText.includes('function renderSeoHubLoadingShell') && jsText.includes("elements.seoHub.setAttribute('aria-busy', 'true')") && jsText.includes("if (!getSeoBundle()) {\n      renderSeoHubLoadingShell({ classicView });\n      await loadSeoLanguageContent(state.language);") && jsText.includes('state.seoOpenRequestToken += 1'), 'Productos debe abrir un shell interactivo inmediatamente mientras hidrata textX/seo, y cancelar de forma segura una apertura pendiente si el usuario cierra el panel.');
  assert(initSource.includes('loadSeoLanguageContent(state.language)') && initSource.indexOf('loadSeoLanguageContent(state.language)') < initSource.indexOf('render();'), 'La escena 1 debe iniciar la precarga SEO del idioma activo antes del primer render interactivo, sin esperar a la tarea diferida posterior.');
  ...
  assert(indexHtml.includes('\"@type\": \"FAQPage\"') && indexHtml.includes(`${PUBLIC_SITE_URL}#faq`) && indexHtml.includes('¿Por qué contratar a Hashinmy?') && indexHtml.includes('¿Para quién es?') && indexHtml.includes('¿Cómo empiezo?'), 'La home debe mantener FAQPage estructurado alineado con el FAQ rápido visible.');
  ...
  assert(llmsText.includes(`${PUBLIC_SITE_URL}es/productos/software-para-ecommerce/`) && llmsText.includes(`${PUBLIC_SITE_URL}en/products/security-privacy/`) && llmsText.includes(`${PUBLIC_SITE_URL}es/productos/cotizador-tecnico-aprovechamiento/`) && llmsText.includes(`${PUBLIC_SITE_URL}en/products/business-chatbots/`), 'llms.txt debe mantener un mapa bilingüe de URLs SEO para buscadores e IA.');
  ...
  assert(indexHtml.includes('<link rel="preload" as="image" href="./assets/hm-scene-01-financing-command.webp" fetchpriority="high"') && !indexHtml.includes('rel="prefetch" as="image" href="./assets/hm-scene-02-service-atlas.webp"'), 'La apertura debe priorizar solo la imagen crítica de la escena 1 desde HTML; las escenas siguientes no deben competir antes de cerrar el cargador inicial.');
  assert(jsText.includes('function getInitialSceneImagePreloadWindow') && jsText.includes('function isInitialPageOpenForSecondaryScenePreloads') && jsText.includes('includeSecondaryScenes = isInitialPageOpenForSecondaryScenePreloads()') && jsText.includes('preloadRemainingSceneImagesAfterOpen(state.scene)'), 'El runtime debe limitar la precarga antes de abrir a la escena inicial/activa y cargar la ventana de escenas siguientes solo después del primer render.');
  ...
  assert(jsText.includes('const fullscreenTarget = document.documentElement;') && !jsText.includes('const fullscreenTarget = elements.stage || document.documentElement;'), 'El modo pantalla completa debe aplicarse al documento completo para mantener visible el acceso global de WhatsApp fuera de .hm-stage.');
  assert(indexHtml.includes('class="hm-whatsapp-float"') && indexHtml.includes('class="hm-whatsapp-float__media"') && buildScript.includes('class="hm-whatsapp-float"'), 'El acceso flotante de WhatsApp debe existir tanto en la experiencia principal como en las páginas estáticas generadas.');
  assert(indexHtml.includes('class="hm-whatsapp-float__outline"') && indexHtml.includes('stroke="#FFFFFF"') && indexHtml.includes('stroke-width="16"') && indexHtml.includes('scale(.95)') && !indexHtml.includes('scale(.82)') && !indexHtml.includes('vector-effect="non-scaling-stroke"') && buildScript.includes('class="hm-whatsapp-float__outline"') && buildScript.includes('stroke="#FFFFFF"') && buildScript.includes('stroke-width="16"') && buildScript.includes('scale(.95)') && !buildScript.includes('scale(.82)') && !buildScript.includes('vector-effect="non-scaling-stroke"') && cssText.includes('.hm-whatsapp-float__outline {') && classicCssText.includes('.hm-whatsapp-float__outline {'), 'El media de WhatsApp debe superponer un contorno blanco proporcional, cercano al borde real del logo, tanto cuando carga la imagen como cuando se usa el fallback.');
  assert(cssText.includes('.hm-whatsapp-float {') && cssText.includes('z-index: 130;') && classicCssText.includes('.hm-whatsapp-float {') && classicCssText.includes('z-index: 130;'), 'El acceso flotante de WhatsApp debe permanecer por encima de loaders, modales y superficies clásicas.');
  ...
  assert(jsText.includes('function detectPreferredLanguage') && jsText.includes('function detectBrowserLanguage') && jsText.includes('function getDefaultInitialLanguage') && jsText.includes("const INITIAL_LANGUAGE_DEFAULT_CODE = 'en';") && jsText.includes('return detectBrowserLanguage() || getDefaultInitialLanguage();') && jsText.includes('const code = resolveKnownLanguageCode(language);') && jsText.includes('if (code) return code;') && !jsText.includes('const code = normalizeLanguageCode(language);\n      if (languageCodes.has(code)) return code;'), 'El autodetector de idioma debe probar todos los locales del navegador y, si el navegador no entrega un idioma soportado, caer a inglés por defecto, no a español.');
  assert(jsText.includes('CONFIGmemoriaBACKEND.json') && jsText.includes('async function loadMemoriaBackendRuntimeConfig') && jsText.includes('applyMemoriaBackendRuntimeConfig') && jsText.includes("MEMORIA_BACKEND_SITE_ID = memoriaSiteId") && jsText.includes('/sdk/memoria.js?s=') && jsText.includes('function resolveLanguageFromMemoriaBackendVisitOpening') && jsText.includes('api.registrarAperturaStatica') && jsText.includes('api.detectarPaisEIdioma') && jsText.includes('api.registerStaticOpen') && jsText.includes('requestVisitOpeningApiDirectly') && jsText.includes('/visitas/apertura') && !jsText.includes(`MEMORIA_BACKEND_SITE_ID = '${MEMORIA_BACKEND_CONFIG.MEMORIA_SITE_ID}'`) && !jsText.includes(`MEMORIA_BACKEND_BASE_URL = '${MEMORIA_BACKEND_CONFIG.MEMORIA_BACKEND_URL}'`) && !jsText.includes(`PUBLIC_SITE_URL = '${MEMORIA_BACKEND_CONFIG.ORIGEN_PROYECTO}'`) && !jsText.includes(['geo', 'Visit', 'Api'].join('')) && !jsText.includes(`/${['geo', 'Visit', 'Api'].join('')}`), 'La apertura debe cargar ORIGEN_PROYECTO, MEMORIA_BACKEND_URL y MEMORIA_SITE_ID desde js/CONFIGmemoriaBACKEND.json antes de consultar POST /api/v1/visitas/apertura de memoriaBACKEND; no debe usar endpoints legados ni definir esos valores en el runtime.');
  assert(jsText.includes('function buildMemoriaBackendStrictPayload') && jsText.includes('textoCliente,') && jsText.includes('opcionesSeleccionadas,') && jsText.includes('idempotencyKey,') && jsText.includes('s: MEMORIA_BACKEND_SITE_ID') && jsText.includes('cliente,') && jsText.includes('mensaje: textoCliente') && jsText.includes('items: []') && jsText.includes('notifyOwner: true') && jsText.includes('autoSchedule: true') && jsText.includes('window.memoriaBACKEND.flujoComercialCotizacion(payload, options)') && jsText.includes('X-MB-Idempotency-Key') && jsText.includes('/flujo-comercial/cotizacion') && !jsText.includes("window.memoriaBACKEND.enviarFormulario('cotizacion_hashinmy'"), 'El envío comercial debe usar solo la API oficial de flujo comercial con el site configurado, conservar textoCliente/opcionesSeleccionadas/idempotencyKey en ese orden, incluir los campos mínimos del contrato y esperar confirmación real de memoriaBACKEND sin caer a enviarFormulario.');
  assert(jsText.includes('const inlineLanguage = getResolvedTextBundleLanguage(inlineBundle') && jsText.includes('if (inlineLanguage === preferred) return inlineBundle;') && !jsText.includes("const spanishCachedBundle = readCachedTextBundle('es');"), 'El arranque rápido no debe pintar el bundle español embebido cuando memoriaBACKEND o el navegador resolvieron otro idioma; debe cargar el bundle solicitado antes del primer render.');
  ...
  assert(jsText.includes('function readStoredRoutePayload') && jsText.includes('function readStoredRouteLanguage') && jsText.includes('language: state.language') && jsText.includes('async function readLanguage') && jsText.includes('await readLanguage();') && jsText.includes("dataset.initialLanguageSource = 'memoriaBACKEND'") && jsText.includes('routeLanguage || (stored ? normalizeLanguageCode(stored) : detectPreferredLanguage())') && jsText.includes("routeLanguage ? 'stored-route' : stored ? 'stored'"), 'El idioma inicial debe resolverse al abrir con prioridad memoriaBACKEND visitas/apertura, luego ruta/idioma guardado y por último navegador o inglés, sin perder la persistencia de idioma en la ruta guardada.');
  ...
  assertSocialPreviewCopySync('es', spanishBundle, 'textX/es.json');
  ...
  for (const code of codes.slice(0, REQUIRED_LANGUAGE_COUNT)) {
    ...
    if (await exists(path.join(root, 'textX', `${code}.json`))) {
      ...
      assertSocialPreviewCopySync(code, bundle);
