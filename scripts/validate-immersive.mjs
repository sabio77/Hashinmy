...
if (!assetsOnly) {
  ...
  assert(jsText.includes('const MEMORIA_BACKEND_SITE_ID = ') && jsText.includes('/sdk/memoria.js?s=') && jsText.includes('function resolveLanguageFromMemoriaBackendVisitOpening') && jsText.includes('api.registrarAperturaStatica') && jsText.includes('api.detectarPaisEIdioma') && jsText.includes('api.registerStaticOpen') && jsText.includes('requestVisitOpeningApiDirectly') && jsText.includes('/visitas/apertura') && !jsText.includes(['geo', 'Visit', 'Api'].join('')) && !jsText.includes(`/${['geo', 'Visit', 'Api'].join('')}`), 'La apertura debe consultar la API oficial POST /api/v1/visitas/apertura de memoriaBACKEND con el site autorizado configurado antes de caer al idioma del navegador; no debe usar el endpoint legado de geovisita ni endpoints alternos.');
  assert(jsText.includes('function getMemoriaBackendCommercialHelper') && jsText.includes('api?.flujoComercialCotizacion') && jsText.includes('flowFunction.call(api, payload, options)') && !jsText.includes("enviarFormulario('cotizacion_hashinmy'"), 'El cierre comercial debe enviar la cotización por SDK oficial window.memoriaBACKEND.flujoComercialCotizacion(payload, { idempotencyKey }) y no degradar a enviarFormulario para esta integración.');
  assert(jsText.includes('function buildMemoriaBackendStrictPayload') && jsText.includes('textoCliente,') && jsText.includes('opcionesSeleccionadas: buildCommercialOptionsPayload(email, phone)') && jsText.includes('idempotencyKey: cleanIdempotencyKey') && jsText.includes('s: MEMORIA_BACKEND_SITE_ID') && jsText.includes('cliente,') && jsText.includes('mensaje: textoCliente') && jsText.includes('items: []') && jsText.includes('notifyOwner: true') && jsText.includes('autoSchedule: true') && jsText.includes('buildMemoriaBackendStrictPayload(email, phone, idempotencyKey)') && jsText.includes('isCommercialSubmissionConfirmed(response)') && jsText.includes('if (response.appNotification?.ok === 1 || response.appNotification?.ok === true) return true;') && !jsText.includes("textoCliente: String(body || '').trim() || buildMemoriaBackendSubmissionText(email, phone)"), 'El payload comercial debe enviar primero textoCliente, opcionesSeleccionadas e idempotencyKey, agregar solo los campos mínimos recomendados por apis.txt para flujo comercial, y solo confirmar éxito cuando memoriaBACKEND devuelva acuse válido.');
  ...
  assert(jsText.includes('function readStoredRoutePayload') && jsText.includes('function readStoredRouteLanguage') && jsText.includes('language: state.language') && jsText.includes('async function readLanguage') && jsText.includes('await readLanguage();') && jsText.includes("dataset.initialLanguageSource = 'memoriaBACKEND'") && jsText.includes("dataset.initialLanguageSource = browserLanguage ? 'browser' : 'default-en'"), 'El idioma inicial debe resolverse al abrir con prioridad memoriaBACKEND visitas/apertura, luego navegador y por último inglés, sin perder la persistencia de idioma en la ruta guardada.');
