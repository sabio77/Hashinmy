...
if (!assetsOnly) {
  ...
  assert(jsText.includes("textoCliente: String(body || '').trim() || buildMemoriaBackendSubmissionText(email, phone)") && jsText.includes('opcionesSeleccionadas: buildCommercialOptionsPayload(email, phone)') && jsText.includes('idempotencyKey') && jsText.includes('isCommercialSubmissionConfirmed(response)'), 'El payload comercial debe conservar textoCliente, opcionesSeleccionadas e idempotencyKey, y solo confirmar éxito cuando memoriaBACKEND devuelva acuse válido.');
