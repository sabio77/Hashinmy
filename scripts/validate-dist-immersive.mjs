...
assert(
  ...
  assert(
    ...
    'dist/index.html debe publicar el copy exacto de la vista previa social para hashinmy.com sin depender del runtime.'
  );
  assert(
  ...
  assertOnlyMiniVistaSocialPreviewImages(indexHtml, 'dist/index.html');
  {
    ...
    assert(
      ...
      'dist/index.html debe ubicar title, canonical y metadatos Open Graph/Twitter antes del bundle hmInitialTextBundle para que WhatsApp y otros crawlers los lean al inicio del HTML.'
    );
  }
  ...
  const distInitSource = jsText.slice(jsText.indexOf('async function init()'), jsText.indexOf('function runInit()'));
  assert(jsText.includes('function bindImmediateSeoEntryAccess()') && jsText.includes("elements.seoHubButton.addEventListener('pointerdown', handleSeoHubEntryPointerDown, { passive: true })") && jsText.includes("elements.seoHubButton.addEventListener('keydown', handleSeoHubEntryKeyDown)") && jsText.includes("elements.seoHubButton.addEventListener('click', handleSeoHubEntryClick)") && distInitSource.includes('bindImmediateSeoEntryAccess();') && distInitSource.includes('const memoriaBackendRuntimeConfigReady = loadMemoriaBackendRuntimeConfig();') && distInitSource.includes('await memoriaBackendRuntimeConfigReady;') && distInitSource.indexOf('bindImmediateSeoEntryAccess();') < distInitSource.indexOf('loadMemoriaBackendRuntimeConfig();') && distInitSource.indexOf('bindImmediateSeoEntryAccess();') < distInitSource.indexOf('await memoriaBackendRuntimeConfigReady;') && distInitSource.indexOf('bindImmediateSeoEntryAccess();') < distInitSource.indexOf('await readLanguage();'), 'dist debe conservar el acceso inmediato de Productos antes de cualquier espera remota de CONFIGmemoriaBACKEND o idioma/visita y aislar su primer gesto del fullscreen global para que el click de escena 1 no dependa del bootstrap general.');
  assert(jsText.includes("const fallbackHref = actionElement?.href || '';") && jsText.includes("Productos no pudo abrir el hub inmediato; se usa la ruta navegable de respaldo.") && jsText.includes('if (fallbackHref) window.location.href = fallbackHref;'), 'dist debe conservar la navegación de respaldo de Productos si el hub inmediato no puede hidratarse.');
  assert(!jsText.includes('function isSeoEntryEnhancedNavigationReady()') && jsText.includes("function handleSeoHubEntryClick(event) {\n    event.preventDefault();\n    event.stopPropagation();\n    openSeoHubFromEntry(event.currentTarget);\n  }"), 'dist debe conservar el primer click de Productos independiente de appReady: abre el shell del hub desde el acceso dedicado y usa el href solo como respaldo si esa apertura falla.');
  ...
  assert(jsText.includes('const fullscreenTarget = document.documentElement;') && !jsText.includes('const fullscreenTarget = elements.stage || document.documentElement;'), 'dist debe solicitar pantalla completa sobre document.documentElement para que el acceso flotante de WhatsApp siga visible en modo fullscreen.');
  assert(indexHtml.includes('class="hm-whatsapp-float"') && indexHtml.includes('class="hm-whatsapp-float__media"') && englishIndexHtml.includes('class="hm-whatsapp-float"') && seoSampleHtml.includes('class="hm-whatsapp-float"') && seoSampleEnHtml.includes('class="hm-whatsapp-float"'), 'dist debe conservar el acceso flotante de WhatsApp en portada, entradas localizadas y páginas SEO clásicas.');
  assert(indexHtml.includes('class="hm-whatsapp-float__outline"') && indexHtml.includes('stroke-width="16"') && indexHtml.includes('scale(.95)') && !indexHtml.includes('scale(.82)') && !indexHtml.includes('vector-effect="non-scaling-stroke"') && englishIndexHtml.includes('class="hm-whatsapp-float__outline"') && englishIndexHtml.includes('stroke-width="16"') && englishIndexHtml.includes('scale(.95)') && !englishIndexHtml.includes('scale(.82)') && !englishIndexHtml.includes('vector-effect="non-scaling-stroke"') && seoSampleHtml.includes('class="hm-whatsapp-float__outline"') && seoSampleHtml.includes('stroke-width="16"') && seoSampleHtml.includes('scale(.95)') && !seoSampleHtml.includes('scale(.82)') && !seoSampleHtml.includes('vector-effect="non-scaling-stroke"') && seoSampleEnHtml.includes('class="hm-whatsapp-float__outline"') && seoSampleEnHtml.includes('stroke-width="16"') && seoSampleEnHtml.includes('scale(.95)') && !seoSampleEnHtml.includes('scale(.82)') && !seoSampleEnHtml.includes('vector-effect="non-scaling-stroke"') && buildScript.includes('class="hm-whatsapp-float__outline"') && buildScript.includes('scale(.95)') && !buildScript.includes('scale(.82)') && !buildScript.includes('vector-effect="non-scaling-stroke"') && cssText.includes('.hm-whatsapp-float__outline {') && classicCssText.includes('.hm-whatsapp-float__outline {'), 'dist debe conservar el contorno blanco proporcional y cercano al borde real de WhatsApp en portada, idiomas y páginas SEO generadas.');
  assert(cssText.includes('.hm-whatsapp-float {') && cssText.includes('z-index: 130;') && classicCssText.includes('.hm-whatsapp-float {') && classicCssText.includes('z-index: 130;'), 'dist CSS debe mantener el acceso flotante de WhatsApp por encima del loader y de las superficies de navegación.');
