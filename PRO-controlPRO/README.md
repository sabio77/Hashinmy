...
El modo `sinBACKEND` no aplica una política paralela. Cada capacidad firmada incluye el `resourceType` y el `permissionProfile` del espacio, y `src/js/p2p-permissions.js` reproduce en la PWA la misma decisión autoritativa que memoriaBACKEND. En el perfil `admin-project-v1`, la raíz presupuestal puede editarse con rol Propietario, Gerente o Admin y solo Propietario o Gerente pueden eliminar el proyecto; crear o modificar proyecciones requiere `projection`, borrar una proyección exige simultáneamente `delete` y `projection`, y crear o retirar vínculos de proyección depende de `projection`.
...
Esta iteración corrige la autoridad de las claves compartidas. Antes, cada dispositivo decidía localmente qué clave AES estaba activa; un equipo autorizado pero atrasado podía redistribuir una clave anterior y una instalación nueva podía activarla temporalmente. Ahora cada espacio expone desde memoriaBACKEND un `encryptionAuthorityVersion`, un `activeEncryptionKeyId` y un `encryptionKeyEpoch` de control. La rotación solo la confirma el propietario mediante comparación atómica de la clave esperada. En una creación administrada desde el panel, el Gerente o Admin creador puede confirmar únicamente la primera clave vacía; después de esa activación inicial, toda rotación vuelve a ser exclusiva del propietario. Solicitudes, sobres y publicaciones con una clave anterior son rechazados.
...
La pantalla autenticada implementa el caso administrativo completo sobre entidades genéricas (`admin.project`, `admin.purchase`, `admin.income` y `admin.projection`). El propietario, un Gerente o un Admin puede crear proyectos desde el panel; cuando crea un Admin o Gerente delegado, memoriaBACKEND mantiene al propietario del panel como propietario del proyecto y replica el rol delegado sin conceder capacidad de eliminación al Admin. El panel de participantes conecta directamente `revoke`, `transfer` y `leave`, confirma las acciones destructivas y no presenta una revocación como criptográficamente completa cuando la rotación de clave falla. Puede sustituirse por otra interfaz de negocio conservando `p2p-client.js`, `p2p-permissions.js`, `p2p-storage.js`, `p2p-durability.js`, `p2p-crypto.js` y el mismo contrato de permisos.
