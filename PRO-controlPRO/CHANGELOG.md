...
- El procesamiento diferido conserva la decisión de aceptar o rechazar, cerca temporalmente el auto-aceptado para evitar respuestas duplicadas y reanuda después la convergencia automática de otras herencias pendientes.
- Se añadió una regresión específica que impide volver a introducir esperas por proyecto dentro de la ruta crítica de aceptación del panel.

## 1.10.5 - 2026-08-06

- Las invitaciones que memoriaBACKEND invalida por revocación, eliminación, cambio de panel o reducción de permisos ya no se interpretan como aceptadas cuando el usuario pulsa **Aceptar** sobre una vista obsoleta.
- El cliente trata el estado autoritativo `cancelled` como cierre sin acceso, refresca el bootstrap y evita crear una réplica provisional o iniciar recuperación para un proyecto que ya no puede compartirse.
- La interfaz muestra un aviso de producción específico en español, inglés y árabe; la regresión de intenciones de invitación cubre el nuevo estado canónico.

## 1.10.4 - 2026-08-06

- Cada card de panel real, propio o compartido, conserva un menú contextual independiente con acceso directo a **Participantes**, invitación condicionada por permisos y salida voluntaria para invitados.
- La vista principal de **Participantes** queda disponible para todos los miembros confirmados: propietarios y gestores pueden retirar invitados; cada invitado puede abandonar por sí mismo un panel o proyecto sin recibir capacidades administrativas.
- La validación estática obligatoria ejecuta ahora la regresión específica del menú, permisos delegables, revocación y abandono, evitando publicar una versión cuya interfaz se separe del contrato P2P existente.
- Se regeneraron las huellas de release para que la actualización PWA distribuya esta integración sin conservar `app.js` obsoleto en caché.
